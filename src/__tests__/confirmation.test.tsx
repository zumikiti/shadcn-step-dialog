...
describe("Confirmation", () => {
  ...
  it("should display form data correctly", () => {
    ...
    // 氏名の確認
    expect(screen.getByTestId("confirmation-name")).toHaveTextContent("山田 太郎");
    
    // 住所の確認
    expect(screen.getByTestId("confirmation-address")).toHaveTextContent("東京都渋谷区1-1-1");
    
    // 電話番号の確認
    expect(screen.getByTestId("confirmation-phone")).toHaveTextContent("090-1234-5678");
    
    // 利用規約同意の確認
    expect(screen.getByTestId("confirmation-agreement")).toHaveTextContent("✓ 同意済み");
  });
  ...
  it("should display agreement as not agreed when false", () => {
    ...
    expect(screen.getByTestId("confirmation-agreement")).toHaveTextContent("✗ 未同意");
  });
  ...
  it("should display instruction text", () => {
    ...
    expect(screen.getByText("上記の内容でよろしければ、送信ボタンを押してください。")).toBeInTheDocument();
  });
  ...
  it("should apply correct styling for agreement status", () => {
    ...
    const agreementElement = screen.getByTestId("confirmation-agreement").firstChild as HTMLElement;
